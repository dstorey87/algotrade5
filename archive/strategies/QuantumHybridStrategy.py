...
class QuantumHybridStrategy(IStrategy):
    ...
    def populate_indicators(
        ...
        dataframe["macd"], dataframe["macdsignal"], _ = (
            self.quantum_optimizer.quantum_macd(dataframe["close"])
        )
        dataframe["quantum_trend"] = self.quantum_optimizer.get_quantum_trend(
            dataframe["close"], dataframe["volume"]
        )

        # EMAs for trend following
        dataframe["ema_9"] = (
            pd.Series(dataframe["close"]).ewm(span=9, adjust=False).mean()
        )
        dataframe["ema_21"] = (
            pd.Series(dataframe["close"]).ewm(span=21, adjust=False).mean()
        )
    ...
    def populate_entry_conditions(
        ...
        dataframe.loc[pd.DataFrame(conditions).all(axis=0), "enter_long"] = 1

        return dataframe

    def populate_exit_conditions(
        self, dataframe: pd.DataFrame, metadata: dict
    ) -> pd.DataFrame:
        conditions = []
        ...
        dataframe.loc[pd.DataFrame(conditions).any(axis=0), "exit_long"] = 1
