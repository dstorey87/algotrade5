...
class DocumentationValidator:
    ...
    def _validate_document_content(
        self, doc_path: Path, required_sections: List[str], critical_keywords: List[str]
    ) -> bool:
        """
        ...
        try:
            ...
            if missing_sections:
                logger.error(
                    f"Missing sections in {doc_path.name}: {', '.join(missing_sections)}"
                )
                return False
            ...
            if missing_keywords:
                logger.error(
                    f"Missing keywords in {doc_path.name}: {', '.join(missing_keywords)}"
                )
                return False
    ...
    def _verify_recent_update(self, content: str) -> bool:
        ...
        try:
            ...
            update_match = re.search(
                r"Last Updated:?\s*(\d{4}-\d{2}-\d{2})", content, re.IGNORECASE
            )
