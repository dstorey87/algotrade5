...
class RiskManager:
    ...
    def calculate_stop_loss(
        self, entry_price: float, current_price: float, atr: Optional[float] = None
    ) -> Tuple[float, str]:
        """
    ...
    def check_trade_risk(self, trade: Trade) -> bool:
        ...
        if current_win_rate < self.win_rate_threshold:
            logger.warning(
                f"Current win rate {current_win_rate:.2%} below threshold {self.win_rate_threshold:.2%}"
            )
            return False
        ...
        if trade.stake_amount > self.calculate_fixed_fractional_size(
            trade.stake_amount
        ):
    ...
    def _trigger_emergency_stop(self) -> None:
        ...
        self.error_manager.log_error(
            "Emergency stop triggered", ErrorSeverity.CRITICAL.value, "RiskManagement"
        )
    ...
    def update_trade_history(self, trade: Trade, is_win: bool) -> None:
        ...
        self.trade_history.append(
            {"trade": trade, "is_win": is_win, "timestamp": datetime.now().isoformat()}
        )
