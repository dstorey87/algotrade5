...
def test_edge_overrides_stoploss(
    ...
    patch_get_signal(freqtrade)
# REMOVED_UNUSED_CODE:     freqtrade.strategy.min_roi_reached = MagicMock(return_value=False)
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    freqtrade = get_patched_freqtradebot(mocker, default_conf_usdt)
# REMOVED_UNUSED_CODE:     freqtrade.protections._protection_handlers[1].global_stop = MagicMock(
# REMOVED_UNUSED_CODE:         return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf")
# REMOVED_UNUSED_CODE:     )
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    freqtrade = FreqtradeBot(default_conf_usdt)
# REMOVED_UNUSED_CODE:     freqtrade.strategy.informative_pairs = inf_pairs
    # patch_get_signal(freqtrade)
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert freqtrade.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order["id"] = "557"
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert freqtrade.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["status"] = "open"
    order["id"] = "5566"
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert freqtrade.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["status"] = "open"
    order["id"] = "5567"
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order["status"] = "open"
    order["id"] = "5568"
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert freqtrade.execute_entry(pair, stake_amount, is_short=is_short)
...
# REMOVED_UNUSED_CODE:     freqtrade.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)
...
# REMOVED_UNUSED_CODE:     trade.orders[-1].filled = trade.orders[-1].remaining
...
def test_handle_trade_roi(
    ...
    freqtrade = get_patched_freqtradebot(mocker, default_conf_usdt)
    patch_get_signal(freqtrade, enter_short=is_short, enter_long=not is_short)
# REMOVED_UNUSED_CODE:     freqtrade.strategy.min_roi_reached = MagicMock(return_value=True)
...
# REMOVED_UNUSED_CODE:     open_trade.orders[0].side = "sell" if is_short else "buy"
...
# REMOVED_UNUSED_CODE:     freqtrade.strategy.ft_check_timed_out = MagicMock(return_value=False)
...
# REMOVED_UNUSED_CODE:     freqtrade.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    freqtrade.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    freqtrade = FreqtradeBot(default_conf_usdt)
# REMOVED_UNUSED_CODE:     freqtrade._notify_enter_cancel = MagicMock()
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_exit_price = lambda **kwargs: 2.25
    freqtrade.execute_trade_exit(
        trade=trade,
...
# REMOVED_UNUSED_CODE:         freqtrade.strategy.ft_stoploss_reached = MagicMock(
# REMOVED_UNUSED_CODE:             return_value=ExitCheckTuple(exit_type=ExitType.NONE)
# REMOVED_UNUSED_CODE:         )
    freqtrade.enter_positions()
    ...
    if handle_second:
# REMOVED_UNUSED_CODE:         freqtrade.strategy.exit_profit_offset = 0.0
        assert freqtrade.handle_trade(trade) is True
...
def test_get_real_amount_multi(
    ...
    fee_reduction_amount,
    expected_fee,
# REMOVED_UNUSED_CODE:     expected_log_amount,
):
...
# REMOVED_UNUSED_CODE:     freqtrade.wallets.get_owned = MagicMock(return_value=entry_order["amount"] * factor)
...
# REMOVED_UNUSED_CODE:     def refresh_latest_ohlcv_mock(pairlist, **kwargs):
# REMOVED_UNUSED_CODE:         ret = {}
# REMOVED_UNUSED_CODE:         for p, tf, ct in pairlist:
# REMOVED_UNUSED_CODE:             if ct == CandleType.MARK:
# REMOVED_UNUSED_CODE:                 ret[(p, tf, ct)] = mark_prices[p]
# REMOVED_UNUSED_CODE:             else:
# REMOVED_UNUSED_CODE:                 ret[(p, tf, ct)] = funding_rates[p]
# REMOVED_UNUSED_CODE: 
# REMOVED_UNUSED_CODE:         return ret
...
# REMOVED_UNUSED_CODE:     def make_sure_its_651(*args, **kwargs):
# REMOVED_UNUSED_CODE:         if args[0] == "650":
# REMOVED_UNUSED_CODE:             return closed_successful_buy_order
# REMOVED_UNUSED_CODE:         if args[0] == "651":
# REMOVED_UNUSED_CODE:             return open_dca_order_1
