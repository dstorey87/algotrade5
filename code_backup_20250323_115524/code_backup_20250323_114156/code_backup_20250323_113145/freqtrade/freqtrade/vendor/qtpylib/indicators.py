...
# REMOVED_UNUSED_CODE: def awesome_oscillator(df, weighted=False, fast=5, slow=34):
# REMOVED_UNUSED_CODE:     midprice = (df["high"] + df["low"]) / 2
# REMOVED_UNUSED_CODE: 
# REMOVED_UNUSED_CODE:     if weighted:
# REMOVED_UNUSED_CODE:         ao = (midprice.ewm(fast).mean() - midprice.ewm(slow).mean()).values
# REMOVED_UNUSED_CODE:     else:
# REMOVED_UNUSED_CODE:         ao = numpy_rolling_mean(midprice, fast) - numpy_rolling_mean(midprice, slow)
# REMOVED_UNUSED_CODE: 
# REMOVED_UNUSED_CODE:     return pd.Series(index=df.index, data=ao)
...
# REMOVED_UNUSED_CODE: PandasObject.zlwma = zlema
