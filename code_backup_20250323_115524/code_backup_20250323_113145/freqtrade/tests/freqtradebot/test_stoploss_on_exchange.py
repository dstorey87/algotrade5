...
def test_handle_stoploss_on_exchange(
    ...
    trade.is_open = False
# REMOVED_UNUSED_CODE:     trade.open_sl_orders[-1].ft_is_open = False
    stoploss.reset_mock()
...
# REMOVED_UNUSED_CODE:     trade.stoploss_last_update = dt_now() - timedelta(hours=1)
    trade.stop_loss = 24
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    trade.is_open = True
    assert trade.has_open_sl_orders is False
# REMOVED_UNUSED_CODE:     trade.stoploss_last_update = dt_now() - timedelta(minutes=20)
    trade.orders.append(
        Order(
            ft_order_side="stoploss",
            ft_pair=trade.pair,
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     price_mock = MagicMock(side_effect=lambda p, s, **kwargs: int(s))
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     freqtrade.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     def fetch_stoploss_order_mock(order_id, *args, **kwargs):
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:         x = deepcopy(slo)
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:         x["id"] = order_id
...
# REMOVED_UNUSED_CODE:     trade.stoploss_last_update = dt_now()
    trade.orders.append(
        Order(
            ft_order_side="stoploss",
            ft_pair=trade.pair,
