...
def test_kucoin_create_order(default_conf, mocker, side, ordertype, rate):
    ...
    default_conf["dry_run"] = False
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     mocker.patch(f"{EXMS}.amount_to_precision", lambda s, x, y: y)
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     mocker.patch(f"{EXMS}.price_to_precision", lambda s, x, y: y)
    exchange = get_patched_exchange(mocker, default_conf, api_mock, exchange="kucoin")
# REMOVED_UNUSED_CODE:     exchange._set_leverage = MagicMock()
# REMOVED_UNUSED_CODE:     exchange.set_margin_mode = MagicMock()
