...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE: from freqtrade.exchange import timeframe_to_next_date, timeframe_to_prev_date
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
# REMOVED_UNUSED_CODE:     mocker.patch("freqtrade.configuration.configuration.create_datadir", lambda c, x: x)
...
# REMOVED_UNUSED_CODE:     def get_timerange(input1):
# REMOVED_UNUSED_CODE:         return dt_utc(2017, 11, 14, 21, 17), dt_utc(2017, 11, 14, 22, 59)
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     backtesting.strategy.custom_stake_amount = lambda **kwargs: 123.5
    backtesting.wallets.update()
    trade = backtesting._enter_trade(pair, row=row, direction="long")
    LocalTrade.bt_trades_open.pop()
    assert trade
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     backtesting.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    trade = backtesting._enter_trade(pair, row=row, direction="long")
    LocalTrade.bt_trades_open.pop()
    assert trade
    assert trade.stake_amount == 495
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch(f"{EXMS}.get_max_pair_stake_amount", return_value=float("inf"))
    mocker.patch(
# REMOVED_UNUSED_CODE:         "freqtrade.persistence.trade_model.price_to_precision", lambda p, *args, **kwargs: p
    )
...
# REMOVED_UNUSED_CODE:     def advise_entry(df, *args, **kwargs):
# REMOVED_UNUSED_CODE:         # Mock function to force several entries
# REMOVED_UNUSED_CODE:         df.loc[(df["rsi"] < 40), "enter_long"] = 1
# REMOVED_UNUSED_CODE:         return df

# REMOVED_UNUSED_CODE:     def custom_entry_price(proposed_rate, **kwargs):
# REMOVED_UNUSED_CODE:         return proposed_rate * 0.997

    default_conf_usdt["max_open_trades"] = 10

    backtesting = Backtesting(default_conf_usdt)
    backtesting._set_strategy(backtesting.strategylist[0])
# REMOVED_UNUSED_CODE:     backtesting.strategy.populate_entry_trend = advise_entry
# REMOVED_UNUSED_CODE:     backtesting.strategy.ignore_buying_expired_candle_after = 59
...
# REMOVED_UNUSED_CODE:     def advise_entry(df, *args, **kwargs):
# REMOVED_UNUSED_CODE:         # Mock function to force several entries
# REMOVED_UNUSED_CODE:         df.loc[:, "enter_long"] = 1
# REMOVED_UNUSED_CODE:         return df

# REMOVED_UNUSED_CODE:     def adjust_trade_position(trade, current_time, **kwargs):
# REMOVED_UNUSED_CODE:         if current_time > datetime(2021, 11, 18, 2, 0, 0, tzinfo=timezone.utc):
# REMOVED_UNUSED_CODE:             return None
# REMOVED_UNUSED_CODE:         return default_conf_usdt["stake_amount"]
...
# REMOVED_UNUSED_CODE:     backtesting.strategy.leverage = lambda **kwargs: 1
# REMOVED_UNUSED_CODE:     backtesting.strategy.position_adjustment_enable = True
    pair = "XRP/USDT:USDT"
    # Pick a timerange adapted to the pair we use to test
    timerange = TimeRange.parse_timerange("20211117-20211119")
    data = history.load_data(
...
def test_backtest_trim_no_data_left(default_conf, fee, mocker, testdatadir) -> None:
    ...
    backtesting._set_strategy(backtesting.strategylist[0])
    timerange = TimeRange("date", None, 1517227800, 0)
# REMOVED_UNUSED_CODE:     backtesting.required_startup = 100
...
# REMOVED_UNUSED_CODE:     def tmp_confirm_entry(pair, current_time, **kwargs):
# REMOVED_UNUSED_CODE:         nonlocal count
# REMOVED_UNUSED_CODE:         dp = backtesting.strategy.dp
# REMOVED_UNUSED_CODE:         df, _ = dp.get_analyzed_dataframe(pair, backtesting.strategy.timeframe)
# REMOVED_UNUSED_CODE:         current_candle = df.iloc[-1].squeeze()
# REMOVED_UNUSED_CODE:         assert current_candle["enter_long"] == 1
# REMOVED_UNUSED_CODE: 
# REMOVED_UNUSED_CODE:         candle_date = timeframe_to_next_date(backtesting.strategy.timeframe, current_candle["date"])
# REMOVED_UNUSED_CODE:         assert candle_date == current_time
# REMOVED_UNUSED_CODE:         # These asserts don't properly raise as they are nested,
# REMOVED_UNUSED_CODE:         # therefore we increment count and assert for that.
# REMOVED_UNUSED_CODE:         df = dp.get_pair_dataframe(pair, backtesting.strategy.timeframe)
# REMOVED_UNUSED_CODE:         prior_time = timeframe_to_prev_date(
# REMOVED_UNUSED_CODE:             backtesting.strategy.timeframe, candle_date - timedelta(seconds=1)
# REMOVED_UNUSED_CODE:         )
# REMOVED_UNUSED_CODE:         assert prior_time == df.iloc[-1].squeeze()["date"]
# REMOVED_UNUSED_CODE:         assert df.iloc[-1].squeeze()["date"] < current_time
# REMOVED_UNUSED_CODE: 
# REMOVED_UNUSED_CODE:         count += 1

# REMOVED_UNUSED_CODE:     backtesting.strategy.confirm_trade_entry = tmp_confirm_entry
    backtesting.backtest(
        processed=deepcopy(processed),
        start_date=min_date,
        end_date=max_date,
...
def test_backtest_pricecontours(
    ...
    min_date, max_date = get_timerange(processed)
    assert isinstance(processed, dict)
# REMOVED_UNUSED_CODE:     backtesting.strategy.max_open_trades = 1
...
def test_backtest_alternate_buy_sell(default_conf, fee, mocker, testdatadir):
    ...
    default_conf["timeframe"] = "1m"
    backtesting = Backtesting(default_conf)
# REMOVED_UNUSED_CODE:     backtesting.required_startup = 0
    backtesting._set_strategy(backtesting.strategylist[0])
...
# REMOVED_UNUSED_CODE: # REMOVED_UNUSED_CODE:     mocker.patch(f"{EXMS}.price_to_precision", lambda s, x, y, **kwargs: y)
    mocker.patch(f"{EXMS}.get_min_pair_stake_amount", return_value=0.00001)
    mocker.patch(f"{EXMS}.get_max_pair_stake_amount", return_value=float("inf"))
    mocker.patch(f"{EXMS}.get_fee", fee)
    patch_exchange(mocker)
    ...
    bl_spy = mocker.spy(backtesting, "backtest_loop")
    backtesting.detail_data = detail_data
# REMOVED_UNUSED_CODE:     backtesting.funding_fee_timeframe_secs = 3600 * 8  # 8h
# REMOVED_UNUSED_CODE:     backtesting.futures_data = {pair: pd.DataFrame() for pair in pairs}

# REMOVED_UNUSED_CODE:     backtesting.strategylist[0].can_short = True
    backtesting._set_strategy(backtesting.strategylist[0])
    backtesting.strategy.bot_loop_start = MagicMock()
    backtesting.strategy.advise_entry = _trend_alternate_hold  # Override
    backtesting.strategy.advise_exit = _trend_alternate_hold  # Override
