...
# REMOVED_UNUSED_CODE: from freqtrade_client import __version__
...
# REMOVED_UNUSED_CODE: def load_config(configfile):
# REMOVED_UNUSED_CODE:     file = Path(configfile)
# REMOVED_UNUSED_CODE:     if file.is_file():
# REMOVED_UNUSED_CODE:         with file.open("r") as f:
# REMOVED_UNUSED_CODE:             config = rapidjson.load(
# REMOVED_UNUSED_CODE:                 f, parse_mode=rapidjson.PM_COMMENTS | rapidjson.PM_TRAILING_COMMAS
# REMOVED_UNUSED_CODE:             )
# REMOVED_UNUSED_CODE:         return config
# REMOVED_UNUSED_CODE:     else:
# REMOVED_UNUSED_CODE:         logger.warning(f"Could not load config file {file}.")
# REMOVED_UNUSED_CODE:         sys.exit(1)


# REMOVED_UNUSED_CODE: def print_commands():
# REMOVED_UNUSED_CODE:     # Print dynamic help for the different commands using the commands doc-strings
# REMOVED_UNUSED_CODE:     client = FtRestClient(None)
# REMOVED_UNUSED_CODE:     print("Possible commands:\n")
# REMOVED_UNUSED_CODE:     for x, _ in inspect.getmembers(client):
# REMOVED_UNUSED_CODE:         if not x.startswith("_"):
# REMOVED_UNUSED_CODE:             doc = re.sub(":return:.*", "", getattr(client, x).__doc__, flags=re.MULTILINE).rstrip()
# REMOVED_UNUSED_CODE:             print(f"{x}\n\t{doc}\n")
