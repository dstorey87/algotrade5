...
class ConfigManager:
    ...
    def validate_config(self) -> Dict[str, bool]:
        ...
        validation_results["trading"] = (
            self.global_config["trading"]["min_win_rate"] >= 0.5
            and self.global_config["trading"]["max_drawdown"] <= 0.2
            and self.global_config["trading"]["risk_per_trade"] <= 0.05
        )

        # Validate model parameters
        validation_results["model"] = (
            self.global_config["model"]["min_accuracy"] >= 0.5
            and self.global_config["model"]["min_confidence"] >= 0.5
        )
