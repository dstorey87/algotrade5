...
import subprocess
import shutil
# REMOVED_UNUSED_CODE: import glob
import logging
...
# REMOVED_UNUSED_CODE: from queue import Queue
...
# REMOVED_UNUSED_CODE:     def save_cache(self):
# REMOVED_UNUSED_CODE:         """Save cache to disk"""
# REMOVED_UNUSED_CODE:         try:
# REMOVED_UNUSED_CODE:             with open(self.cache_file, 'wb') as f:
# REMOVED_UNUSED_CODE:                 pickle.dump(self.cache, f)
# REMOVED_UNUSED_CODE:         except Exception as e:
# REMOVED_UNUSED_CODE:             pass  # Gracefully handle cache save failures
...
# REMOVED_UNUSED_CODE:     def clear_cache(self):
# REMOVED_UNUSED_CODE:         """Clear the entire cache"""
# REMOVED_UNUSED_CODE:         with self.lock:
# REMOVED_UNUSED_CODE:             self.cache = {}
# REMOVED_UNUSED_CODE:             if self.cache_file.exists():
# REMOVED_UNUSED_CODE:                 self.cache_file.unlink()
...
# REMOVED_UNUSED_CODE: def is_path_excluded(path: Path, exclude_dirs: List[str]) -> bool:
# REMOVED_UNUSED_CODE:     """Check if a path should be excluded based on any part of its path"""
# REMOVED_UNUSED_CODE:     # Convert path to parts for comparison
# REMOVED_UNUSED_CODE:     path_parts = path.parts
# REMOVED_UNUSED_CODE:     
# REMOVED_UNUSED_CODE:     # Check if any part of the path matches an excluded dir
# REMOVED_UNUSED_CODE:     for exclude_dir in exclude_dirs:
# REMOVED_UNUSED_CODE:         exclude_parts = Path(exclude_dir).parts
# REMOVED_UNUSED_CODE:         
# REMOVED_UNUSED_CODE:         # Check if the exclude pattern appears anywhere in the path
# REMOVED_UNUSED_CODE:         for i in range(len(path_parts) - len(exclude_parts) + 1):
# REMOVED_UNUSED_CODE:             if path_parts[i:i+len(exclude_parts)] == exclude_parts:
...
def get_python_files(exclude_dirs: List[str] = None, exclude_files: List[str] = None) -> List[Path]:
    ...
    python_files = []
# REMOVED_UNUSED_CODE:     file_lock = Lock()
...
# REMOVED_UNUSED_CODE:                     dir_name = entry.name
...
# REMOVED_UNUSED_CODE:         self.last_scan_time = 0
# REMOVED_UNUSED_CODE:         self.debounce_seconds = 5  # Minimum time between scans
    
# REMOVED_UNUSED_CODE:     def on_modified(self, event):
# REMOVED_UNUSED_CODE:         # Only respond to Python file changes
# REMOVED_UNUSED_CODE:         if not event.is_directory and event.src_path.endswith('.py'):
# REMOVED_UNUSED_CODE:             current_time = time.time()
# REMOVED_UNUSED_CODE:             
# REMOVED_UNUSED_CODE:             # Debounce to avoid too frequent scans
# REMOVED_UNUSED_CODE:             if current_time - self.last_scan_time > self.debounce_seconds:
# REMOVED_UNUSED_CODE:                 self.last_scan_time = current_time
# REMOVED_UNUSED_CODE:                 logger.info(f"Change detected in {event.src_path}, running scan...")
# REMOVED_UNUSED_CODE:                 run_full_scan(auto_fix=self.auto_fix)
    
# REMOVED_UNUSED_CODE:     def on_created(self, event):
# REMOVED_UNUSED_CODE:         if not event.is_directory and event.src_path.endswith('.py'):
# REMOVED_UNUSED_CODE:             current_time = time.time()
# REMOVED_UNUSED_CODE:             if current_time - self.last_scan_time > self.debounce_seconds:
# REMOVED_UNUSED_CODE:                 self.last_scan_time = current_time
# REMOVED_UNUSED_CODE:                 logger.info(f"New file detected: {event.src_path}, running scan...")
# REMOVED_UNUSED_CODE:                 run_full_scan(auto_fix=self.auto_fix)
