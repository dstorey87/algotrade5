...
class DependencyManager:
    ...
    def cleanup_cache(self):
        ...
        if current_size >= self.config["build_context"]["cleanup_trigger_gb"]:
            logger.info(
                f"Cache size ({current_size:.2f}GB) exceeds threshold, cleaning..."
            )
        ...
        for group_name, packages in self.config["package_groups"].items():
            ...
            for package, version in packages.items():
                cmd = [
                    ...
                    "-t",
                    str(
                        self.project_root / self.config["cache_paths"]["site-packages"]
                    ),
            ...
            for package, version in group.items():
                ...
                try:
                    ...
                    if pkg_version != version:
                        logger.error(
                            f"Version mismatch for {package}: {pkg_version} != {version}"
                        )
                        return False
                except:
