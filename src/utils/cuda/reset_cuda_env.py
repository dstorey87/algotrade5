...
def reset_cuda_environment():
    ...
    try:
        ...
        packages_to_remove = [
            ...
            "cupy-cuda*", "pennylane-lightning-gpu",
        ...
        for package in packages_to_remove:
            try:
                subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", package],
                             check=False)  # Don't check return code as some might not be installed
        ...
        try:
            ...
            logger.warning("Failed to install CuPy via pip, trying alternative source...")
            try:
                subprocess.run([
                    sys.executable, "-m", "pip", "install",
                    "--no-cache-dir",
                    "--index-url", "https://pypi.anaconda.org/skipper/simple",
                    "cupy-cuda118"
                ], check=True)
            except subprocess.CalledProcessError:
                logger.error("Could not install CuPy. Some GPU operations may be slower.")
        ...
        try:
            ...
            logger.warning("Could not install pennylane-lightning-gpu. GPU acceleration for quantum operations may not be available.")
