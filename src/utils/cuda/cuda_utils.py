...
from src.core.error_manager import get_error_manager, ErrorSeverity
...
class CUDAUtils:
    ...
    def get_memory_info(self) -> Dict[str, int]:
        ...
        try:
            ...
            self.error_manager.log_error(
                f"Failed to get GPU memory info: {e}",
    ...
    def measure_execution_time(self, func: callable, *args, **kwargs) -> Tuple[float, Optional[Exception]]:
    ...
    def run_stress_test(self, size: int = 5000) -> Dict:
        ...
        try:
            ...
            matmul_time, matmul_error = self.measure_execution_time(
                torch.matmul, x, y
            )
            results['matmul'] = {
                'time': matmul_time,
            ...
            if size <= 1000:  # Avoid memory issues with large sizes
                ...
                conv_time, conv_error = self.measure_execution_time(
                    conv, x
                )
                results['conv'] = {
                    'time': conv_time,
