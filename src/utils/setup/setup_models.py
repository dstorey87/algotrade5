...
class ModelSetup:
    ...
    def __init__(self):
        ...
        self.models_path = Path(self.config.get('models_path', 'C:/AlgoTradPro5/models'))
    ...
    def _prepare_directories(self) -> bool:
        ...
        try:
            ...
            self.error_manager.log_error(
                f"Directory preparation failed: {e}",
                    ...
                    try:
                        ...
                        if config_file.exists():
                            ...
                            self.model_info[model_dir.name] = {
                                'config': config,
                                'size': sum(f.stat().st_size for f in model_dir.glob('*')),
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            temp_dir = self.models_path / 'temp'
            if temp_dir.exists():
                shutil.rmtree(temp_dir)
                
            logger.info("Model setup cleanup complete")
            
        except Exception as e:
            self.error_manager.log_error(
                f"Model cleanup failed: {e}",
                ErrorSeverity.MEDIUM.value,
                "Models"
            )
