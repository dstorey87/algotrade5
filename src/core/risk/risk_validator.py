...
class RiskValidator:
    ...
    def validate_drawdown(
        self, current_drawdown: float, peak_capital: float, current_capital: float
    ) -> ValidationResult:
    ...
    def validate_win_rate(
        self, wins: int, total_trades: int, window_size: Optional[int] = None
    ) -> ValidationResult:
        """
    ...
    def validate_portfolio_risk(
        self, positions: List[Dict], account_size: float
    ) -> ValidationResult:
        """Validate overall portfolio risk"""
        # Calculate total risk exposure
        total_risk = sum(p["risk_amount"] for p in positions if p["risk_amount"] > 0)
