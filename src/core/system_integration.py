...
from src.core.initialize_system import get_system_status, initialize_system
...
class SystemIntegration:
    ...
    def _monitor_system_health(self) -> None:
        ...
        while self._running:
            ...
            threading.Event().wait(float(self.config.get("MONITOR_INTERVAL", 5)))
    ...
    def _monitor_gpu(self) -> None:
        ...
        while self._running:
            try:
                ...
                self.error_manager.log_error(
                    f"GPU monitoring error: {e}", ErrorSeverity.MEDIUM.value, "GPU"
                )

            # Sleep for GPU check interval
            threading.Event().wait(float(self.config.get("GPU_CHECK_INTERVAL", 10)))
    ...
    def _handle_shutdown(self, signum: int, frame) -> None:
        ...
        try:
            ...
            self.error_manager.log_error(
                f"Error during shutdown: {e}", ErrorSeverity.HIGH.value, "Shutdown"
            )
