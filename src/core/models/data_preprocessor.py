...
class DataPreprocessor:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.imputer = SimpleImputer(
            strategy=self.config.get("impute_strategy", "mean"), missing_values=np.nan
        )
    ...
    def _initialize_scaler(self) -> Union[StandardScaler, RobustScaler]:
        ...
        if self.scaler_type == "standard":
            ...
            self.error_manager.log_error(
                error_msg, ErrorSeverity.HIGH.value, "Preprocessing"
            )
    ...
    def create_sequences(
        ...
        try:
            ...
            for i in range(
                n_samples - self.sequence_length - self.prediction_horizon + 1
            ):
