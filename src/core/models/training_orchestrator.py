...
class TrainingOrchestrator:
    def __init__(self, config: Dict):
        ...
        self.model_factory = ModelFactory(base_path=self.base_path / "models")

        self.preprocessor = DataPreprocessor(config=config.get("preprocessor", {}))

        self.validator = PredictionValidator(config=config.get("validator", {}))
    ...
    def train(
        ...
        # Create sequences
        X, y = self.preprocessor.create_sequences(processed_data, shuffle=True)
        ...
        # Create model
        model = self.model_factory.create_model(model_type, config=model_config)
        ...
        for epoch in range(self.max_epochs):
                ...
                metrics = self.validator.validate_prediction(
                    {
                        "value": float(pred),
                        "confidence": 0.9,
                    },  # TODO: Add real confidence
                    float(actual),
                )
                validation_results.append(metrics)

            validation_rate = np.mean([m.validation_passed for m in validation_results])
            ...
            if val_loss < best_val_loss:
                ...
                self.model_factory.save_model_weights(
                    model, self.best_model_path.stem, overwrite=True
                )
            else:
    ...
    def _calculate_ensemble_loss(
        ...
        if "roi" in predictions:
            roi_loss = torch.nn.functional.mse_loss(predictions["roi"], targets)
            total_loss += 0.5 * roi_loss  # 50% weight

        # Volatility prediction loss
        if "volatility" in predictions:
            vol_loss = torch.nn.functional.mse_loss(predictions["volatility"], targets)
        ...
        if "trend" in predictions:
            trend_loss = torch.nn.functional.cross_entropy(
                predictions["trend"], targets.long()
            )
