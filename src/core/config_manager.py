...
class ConfigManager:
    ...
    def _load_global_config(self) -> None:
        ...
        self.global_config['freqai'] = {
            ...
            'purge_old_models': os.getenv('FREQAI_PURGE_OLD_MODELS', 'true').lower() == 'true',
        ...
        self.global_config['database'] = {
            ...
            'url_dryrun': os.getenv('DB_URL_DRYRUN', 'sqlite:///data/tradesv3.dryrun.sqlite'),
        }
        ...
        if config_file.exists():
            try:
                with open(config_file, 'r') as f:
                    ...
                    logger.debug(f"Loaded configuration for component: {component_name}")
                    return config
            except Exception as e:
                logger.error(f"Error loading component config for {component_name}: {e}")
    ...
    def get_component_value(self, component: str, key: str, default: Any = None) -> Any:
        """Get configuration value from component config"""
        component_config = self.load_component_config(component)
    ...
    def set_component_value(self, component: str, key: str, value: Any) -> None:
        """Set configuration value in component config"""
        component_config = self.load_component_config(component)
        component_config[key] = value
        self.save_component_config(component, component_config)
    ...
    def validate_config(self) -> Dict[str, bool]:
        ...
        validation_results['trading'] = (
            self.global_config['trading']['min_win_rate'] >= 0.5 and
            self.global_config['trading']['max_drawdown'] <= 0.2 and
            self.global_config['trading']['risk_per_trade'] <= 0.05
        )
        
        # Validate model parameters
        validation_results['model'] = (
            self.global_config['model']['min_accuracy'] >= 0.5 and
            self.global_config['model']['min_confidence'] >= 0.5
        )
        
        # Validate paths
        validation_results['paths'] = all(
            Path(p).exists() or Path(p).parent.exists() 
            for p in self.global_config['paths'].values()
        )
