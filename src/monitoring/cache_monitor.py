...
class CacheMonitor:
    ...
    def _calculate_performance_score(self, stats: Dict) -> float:
        ...
        usage_score = max(0, 100 - stats['usage_percent']) / 100  # Lower usage is better
        file_score = min(1.0, 1000 / max(1, stats['total_files']))  # Fewer files is better
    ...
    def predict_cleanup_needed(self, cache_type: str, hours_ahead: int = 24) -> bool:
        ...
        try:
            ...
            # Current usage
            current_usage = self.cache_manager.get_cache_stats()[cache_type]['usage_percent']
