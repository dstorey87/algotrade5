...
class CacheCleanupScheduler(threading.Thread):
    ...
    def schedule_cleanup(self, cache_type: str, priority: int,
            ...
            for cache_type, stats in cache_stats.items():
                ...
                if usage >= self.cleanup_thresholds['critical']:
                    ...
                    self.schedule_cleanup(
                        cache_type,
                        priority=1,
                        scheduled_time=current_time + timedelta(hours=self.schedule_windows['high'])
                    )
                elif usage >= self.cleanup_thresholds['medium']:
                    self.schedule_cleanup(
                        cache_type,
                        priority=2,
                        scheduled_time=current_time + timedelta(hours=self.schedule_windows['medium'])
                    )
                elif usage >= self.cleanup_thresholds['low']:
                    self.schedule_cleanup(
                        cache_type,
                        priority=3,
                        scheduled_time=current_time + timedelta(hours=self.schedule_windows['low'])
                    )
    ...
    def _verify_cleanup(self, cache_type: str) -> None:
        ...
        try:
            ...
            if cache_type not in stats:
                logger.warning(f"Could not verify cleanup for {cache_type} - cache not found")
                return
            ...
            if usage >= self.cleanup_thresholds['high']:
                logger.warning(f"Cleanup may not have been effective for {cache_type} - usage still at {usage:.1f}%")
