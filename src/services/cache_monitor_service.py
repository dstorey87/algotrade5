...
class CacheMonitorService(threading.Thread):
        ...
        try:
            ...
            for cache_type, stats in cache_stats.items():
                ...
                if current_usage > 90:
                    ...
                    self._send_alert(cache_type, 'WARNING', f"Cache usage high: {current_usage:.1f}%")
    ...
    def _run_maintenance(self) -> None:
        ...
        try:
            ...
            if (current_time - self._last_check).total_seconds() >= self.alert_settings['cleanup_interval']:
    ...
    def _send_alert(self, cache_type: str, severity: str, message: str) -> None:
        ...
        # Check cooldown
        if (alert_key in self._last_alert and 
            (current_time - self._last_alert[alert_key]).total_seconds() < self.alert_settings['alert_cooldown']):
            return
...
def get_cache_monitor_service():
    """Get global cache monitor service instance"""
    global _cache_monitor_service
    if _cache_monitor_service is None:
        _cache_monitor_service = CacheMonitorService()
    return _cache_monitor_service