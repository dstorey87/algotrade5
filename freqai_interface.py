...
class FreqAIInterface:
    ...
    def analyze_trade_patterns(self, trades_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for pattern in self._extract_patterns(trades_df):
                pattern_trades = trades_df[trades_df['patterns'].str.contains(pattern, na=False)]
                
                if len(pattern_trades) < self.min_success_trades:
                    continue
                    
                win_rate = len(pattern_trades[pattern_trades['profit_ratio'] > 0]) / len(pattern_trades)
                ...
                if win_rate >= self.min_pattern_confidence:
                    patterns_analysis[pattern] = {
                        ...
                        'successful_trades': len(pattern_trades[pattern_trades['profit_ratio'] > 0]),
                    ...
                    self.data_manager.catalog_successful_pattern(patterns_analysis[pattern])
    ...
    def get_optimal_patterns(self, market_regime: Optional[str] = None) -> pd.DataFrame:
        ...
        return self.data_manager.get_top_patterns(
    ...
    def get_performance_report(self) -> Dict:
        ...
        try:
            report = {
                ...
                'market_regime_summary': self.data_manager.get_market_regime_summary(
                ).to_dict('records'),
